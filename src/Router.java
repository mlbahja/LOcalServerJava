...
public class Router {
    // Change this line to use ConfigLoader.Route
    ...
    // Update parameter and return types in match method
    public RouteMatch match(HttpRequest request) {
        String path = request.getPath();
        
        for (ConfigLoader.Route route : routes) {  // Use ConfigLoader.Route
            if (path.startsWith(route.getPath())) {
                if (route.getRedirect() != null) {
                    return new RouteMatch(route, RouteMatch.Type.REDIRECT);
                }
    ...
    // Update RouteMatch class to use ConfigLoader.Route
    public static class RouteMatch {
        public enum Type {
            STATIC,
            CGI,
        ...
        private ConfigLoader.Route route;  // Use ConfigLoader.Route
        private Type type;
